...
- Dashboard 页面打开期间由独立滚动心跳保持本机服务，页面每 15 分钟自动刷新，并每 15 分钟增量同步微信与飞书群聊、私信；全部页面关闭后约 3 分钟退出。每轮同步完整完成后才能运行 Terra，语义分析循环只存在于仍运行的当前 Agent 任务中，任务结束后停止模型分析；
- Skill 使用 macOS 默认浏览器打开本机页面；已有完全相同的 Dashboard 标签页时直接刷新并激活，只有未找到时才新开一页；

...
try {
  ...
  const included = trackedFiles().filter((path) => !excludedFromTransfer(path));
...
function assertCleanWorktree() {
  ...
  if (status) {
    throw new Error('Refusing to package a dirty worktree. Commit the complete project first.');
  }
}

...
- 按需服务使用随机 session ID、首次 Skill 打开宽限和滚动页面租约。首次心跳必须在打开宽限内到达；之后活跃页面可独立续期，过期的页面租约不能被复活。所有页面关闭后约 3 分钟退出；失败或不完整的本机读取至少等待 10 分钟才会由页面心跳再次尝试，并且不会启动本轮 Terra 分析；没有 LaunchAgent 或常驻调度器。

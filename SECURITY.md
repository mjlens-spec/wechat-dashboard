...
- 自定义关键词采用 AES-256-GCM 加密，来源范围只允许微信、飞书或双端；群聊星标只保存为本机关系元数据。关键词新增、来源更新、删除和星标变更均通过同源写 API。
...
- 群媒体默认关闭；开启后仍限制单轮数量、文件体积、视频时长与抽帧数。微信群图片和视频使用群 ID、消息本地 ID、精确时间戳及消息缓存内资源指纹做确定性映射；视频必须再由本机 hardlink 索引唯一定位已缓存 MP4。hardlink 密钥只从权限为 `0600` 的 `~/.wx-cli/all_keys.json` 读取，加密索引只在本轮 `0700` 目录内生成 `0600` 临时副本并立即删除。资源指纹多义、附件缺失、路径越界、符号链接、类型或时长校验失败时拒绝输出。下载、解密或抽帧后的文件必须位于本轮私有目录，并校验 JPEG / PNG / GIF / WebP / MP4 文件头。模型只能使用实际检查过的图片或视频帧，导入成功后删除媒体副本和抽帧。
- 按需服务使用随机 session ID、首次 Skill 打开宽限和滚动页面租约。首次心跳必须在打开宽限内到达；之后活跃页面可独立续期，过期的页面租约不能被复活。所有页面关闭后约 3 分钟退出；页面每 15 分钟重新加载，失败或不完整的本机读取至少等待 15 分钟才会由页面心跳再次尝试，并且不会启动本轮 Terra 分析；没有 LaunchAgent 或常驻调度器。
- Skill 读取 macOS 默认浏览器设置，并仅对已知支持脚本化标签页的 Safari / Chromium 系浏览器检查完全相同的 loopback 页面。命中时把同一标签页重新导航到原 URL 以刷新；未命中时才调用 macOS 默认浏览器打开 URL。检查失败或浏览器不受支持时停止，避免盲目重复开页。

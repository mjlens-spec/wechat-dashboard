...
- 任意 Dashboard 页面打开时，本机页面心跳会在同步到期后触发微信与飞书的群聊、私信新增消息读取；频率约为每 15 分钟一次，每小时做一次时间戳对账。
- 页面每 30 秒向本机服务发送一次心跳。首次页面必须在 Skill 打开宽限内连接；连接成功后，页面心跳独立滚动续期，不再受初始 15 分钟宽限限制。所有页面关闭后，临时服务约 3 分钟内自动退出。
- 每个已打开的 Dashboard 页面每 15 分钟自动重新加载一次；页面从后台重新可见时，如果已经超过刷新时间，也会立即重新加载。
- 双端同步完成后，当前打开的总览、会话汇总、重点关注、潜在商机和关键词页面会立即回读本机结果；每分钟轮询与重新聚焦页面仍作为补偿刷新。Terra High 结果导入后，页面最迟在下一次 30 秒心跳时自动更新。
- 模型语义分析只存在于当前仍在运行、且可使用 Terra High 的 Codex 任务中。任务结束后不会继续后台分析。
- 再次使用时重新调用 Skill；也可以显式运行 `pnpm session:stop`。

这意味着本地页面服务和 AI 分析是两个边界：默认浏览器中的 Dashboard 页面负责维持临时本机监听，并每 15 分钟刷新页面和双端消息；当前 Agent 任务在同步完整完成后负责 Terra High 语义分析。没有常驻调度器时，任务结束后无法继续每 15 分钟调用模型。

## 当前能力

- 微信读取本机会话元数据；飞书通过用户认证读取会话，统一区分平台、群聊与私信。
- 首次同步先建立元数据快照，再分批补齐最近 2 小时和当天消息。
- 页面打开期间每 15 分钟重新加载并增量同步群聊和私信；提供“立即刷新”和“继续补齐今天”。
- 展示消息量、活跃群聊、活跃私信、趋势、近期会话和同步覆盖率。
- 总览提供“优先群聊”工作区：群聊可手动星标置顶；优先关键词命中群名或当前统计区间消息时自动前置；搜索可以同时检索群名和本机已同步的区间消息。
- 总览首屏采用左右双栏：左侧微信、右侧飞书，每栏同时呈现群聊与私信活跃度、待关注事项、同步覆盖和优先群聊；窄屏会自动改为上下排列。
- 优先级顺序固定为“星标置顶 → 关键词命中数 → 当前区间消息量 → 最近活跃时间”。关键词使用 AES-256-GCM 加密保存，搜索与排序只在本机完成。
- 本机设置可创建自定义关键词标签，并逐项选择微信、飞书或双端来源；左侧边栏会为每个关键词生成独立页面，按时间范围展示命中趋势、相关会话和有界消息片段。
...
- 重点关注与潜在商机一旦被处理、跟进、转化或忽略，后续分析会按同会话证据与事项语义持续抑制，不再生成措辞不同的重复卡片；在历史列表中恢复为待处理后可重新提示。
- 语义分析只接受 Terra High（`gpt-5.6-terra` + reasoning `high`）。
- 群媒体分析需要在 `/setup` 单独开启。微信群图片按“群 ID + 消息本地 ID + 精确时间戳”从只读消息缓存取得资源指纹，再匹配该群本机附件目录中的原图、高清图或缩略图。微信群视频沿用同一消息级精确映射，再通过本机只读 hardlink 索引找到唯一的已缓存 MP4，最多抽取 6 帧做视觉与 OCR 分析。飞书群图片与视频也已接通；视频仅分析抽样画面，不转写音频。
...
- 视频抽帧需要本机可执行 `ffmpeg` 与 `ffprobe`；缺失时只跳过对应视频，不影响文本和图片分析。
...
每次调用 Skill 时都会先启动临时本机服务，再检查 macOS 默认浏览器中是否已有完全相同的 Dashboard 页面：已有时直接刷新并激活该标签页，未找到时才用默认浏览器新开一页；明确要求“停止”时例外。15 分钟参数只负责首次打开页面的宽限；页面开始发送心跳后可持续查看、自动刷新和同步，关闭全部页面后临时服务约 3 分钟内退出。
...
```bash
# 启动临时服务，默认给 15 分钟打开页面的宽限期
pnpm session:start

# 在默认浏览器复用并刷新已有 Dashboard 页面，或在未打开时新开一页
pnpm session:open
...
4. 任意 Dashboard 页面打开时，全局页面心跳会在 15 分钟同步到期后调用一次双端增量读取；停留在总览、会话汇总、关键词、商机或重点关注页面均有效。
...
1. 启动本机临时 Dashboard；在 macOS 默认浏览器中复用并刷新已有同一页面，或在未打开时新开一页，然后增量同步消息。
2. 从当天双端会话导出有界上下文：最多 80 个会话、每会话 160 条、总计 800 条，每条正文最多 1200 字符；私信和群媒体分别要求显式开启。
3. 群媒体开启后，每轮最多处理 8 个媒体对象、源文件合计最多 200 MB；图片最大 12 MB，视频最大 80 MB、最长 15 分钟，每个视频最多抽取 6 帧。模型只能引用实际打开过的图片或视频帧，不得从不可用占位符推断内容。
4. Skill 优先创建隔离的 `gpt-5.6-terra`、reasoning `high` 执行单元，按会话独立生成当天汇总、重点关注提示和潜在商机；宿主无法提供该组合时停止语义导入。
5. 每条结论引用匿名 evidence ID；导入时验证 evidence ID 属于同一任务和同一会话。
6. 结果继续加密落盘；临时上下文、媒体副本、视频帧和结果文件在成功导入后删除。

Skill 默认让当前支持 Terra High 的 Codex 任务在页面打开期间继续活动，并使用 25 分钟任务租约覆盖下一次 15 分钟语义循环；用户明确要求只运行一次时例外。每轮先完成微信与飞书的群聊、私信增量同步；只有同步状态为完整成功，才导出受限上下文并运行 Terra High。页面关闭后停止，不自动重启。这个模型循环不会跨越当前 Agent 任务，也不会创建定时 Automation。任务结束后，模型分析停止；本机页面仍可在保持打开时继续 15 分钟页面刷新和增量同步，关闭全部页面后自动退出。
...
脚本会打包当前受控源码快照，包括本次功能新增文件；若工作树尚未提交，清单会明确标注 `working-tree-snapshot` 和基础提交，不会伪装成干净提交。包内同时生成首次安装说明、跨路径 Codex 覆盖升级说明、相对路径源码清单、检查脚本、`SHA256SUMS.txt` 和 ZIP 外部校验文件。输出位于项目内可见的 `交付包/`；本机数据库、密钥、日志、构建缓存、项目 Handoff、用户资料和历史 ZIP 不会进入包内。完整移交边界见 [TRANSFER.md](TRANSFER.md)。

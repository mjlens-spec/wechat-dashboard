...
- 任意 Dashboard 页面打开时，本机页面心跳会在同步到期后触发微信与飞书的群聊、私信新增消息读取；频率约为每 10 分钟一次，每小时做一次时间戳对账。
- 页面每 30 秒向本机服务发送一次心跳。首次页面必须在 Skill 打开宽限内连接；连接成功后，页面心跳独立滚动续期，不再受初始 10 分钟宽限限制。所有页面关闭后，临时服务约 3 分钟内自动退出。
...
这意味着本地页面服务和 AI 分析是两个边界：Chrome 页面负责维持临时本机监听，并每 10 分钟刷新双端消息；当前 Agent 任务在同步完整完成后负责 Terra High 语义分析。没有常驻调度器时，任务结束后无法继续每 10 分钟调用模型。

## 当前能力

- 微信读取本机会话元数据；飞书通过用户认证读取会话，统一区分平台、群聊与私信。
- 首次同步先建立元数据快照，再分批补齐最近 2 小时和当天消息。
- 页面打开期间每 10 分钟增量同步群聊和私信；提供“立即刷新”和“继续补齐今天”。
...
每次调用 Skill 时都会先启动临时本机服务，并自动在内置浏览器或 Google Chrome 中打开 Dashboard；明确要求“停止”时例外。10 分钟参数只负责首次打开页面的宽限；页面开始发送心跳后可持续查看和自动同步，关闭全部页面后临时服务约 3 分钟内退出。
...
4. 任意 Dashboard 页面打开时，全局页面心跳会在 10 分钟同步到期后调用一次双端增量读取；停留在总览、会话汇总、关键词、商机或重点关注页面均有效。
...
用户明确要求“持续监控”时，Skill 可以让当前支持 Terra High 的 Codex 任务保持活动，并使用 20 分钟任务租约覆盖下一次 10 分钟语义循环。每轮先完成微信与飞书的群聊、私信增量同步；只有同步状态为完整成功，才导出受限上下文并运行 Terra High。页面关闭后停止，不自动重启。这个模型循环不会跨越当前 Agent 任务，也不会创建定时 Automation。任务结束后，模型分析停止；本机页面仍可在保持打开时继续 10 分钟增量同步，关闭全部页面后自动退出。
